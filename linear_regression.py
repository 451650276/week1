...
for i in range(m):
    ...
    x1.append(float(list[i][0]))
    x2.append(float(list[i][1]))
    y.append(float(list[i][2]))

theta0,theta1,theta2=5004.24,22000.52,50000.55
sum1,sum2=sum(x1),sum(x2)
...
LR=float(raw_input("your Learning Rate:"))
J=0.0
for i in range(m):
    J=J+((theta0+x1[i]*theta1+x2[i]*theta2)-y[i])**2
J=J/(2*m)
LX=2200000000
while(J>=LX):
    PastJ=J
